...
<#
	...
	.PARAMETER WithID
		Only apply changes if the input object has an ID property.
		Since most uses are only needed intended when there is an ID object in the output, this is a custom simplification specific to this module.
...
#>
	...
	Process
	{
		...
		{
			if ((-not $WithID) -or ($Object.PSObject.Properties.Name -eq "ID"))
			{
